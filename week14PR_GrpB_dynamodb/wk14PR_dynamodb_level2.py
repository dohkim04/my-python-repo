...
"""
##### Retrieve individual dictionary representing each movie item using Scan method ####
import boto3
from boto3.dynamodb.conditions import Key, Attr
...
# Use boto3 and Python to query a table, remove an item from a table, create a table, and delete a table.
# [2.1] Query a table 
# [2.2] Remove an item from a table
# [2.3] Create a table and add items on to the table => shown in Foundation portion 
# [2.4] Delete a table

##########
'''
## [2.1] Query a table : pull out all the movie information based on the release year 1939.
import boto3
from boto3.dynamodb.conditions import Key, Attr
dynamodb = boto3.resource('dynamodb') # Get the service resource.
table = dynamodb.Table('MyMovieList') # call MyMovieList DynamoDB table
response = table.query(
    IndexName='new_search',
    #TableName='MyMovieList' #DynamoDB table name was already designated above.
    KeyConditionExpression = Key('year').eq(1939),
)
items = response['Items']
for item in items:
    print(item)
'''
'''Output:
~/my-python-repo/week14PR_GrpB_dynamodb$ python3.10 wk14PR_dynamodb_level2.py 
...
table = dynamodb.Table('MyMovieList')
...
~/my-python-repo/week14PR_GrpB_dynamodb$ python3.10 wk14PR_dynamodb_level2.py 
dynamodb.Table(name='MyMovieList')
{'ResponseMetadata': {'RequestId': 'GVQKBOQOHED8BVM1DBH5KTP85RVV4KQNSO5AEMVJF66Q9ASUAAJG', 'HTTPStatusCode': 200, 'HTTPHeaders': {'server': 'Server', 'date': 'Wed, 19 Apr 2023 03:23:37 GMT', 'content-type': 'application/x-amz-json-1.0', 'content-length': '2', 'connection': 'keep-alive', 'x-amzn-requestid': 'GVQKBOQOHED8BVM1DBH5KTP85RVV4KQNSO5AEMVJF66Q9ASUAAJG', 'x-amz-crc32': '2745614147'}, 'RetryAttempts': 0}}
'''


##########
"""
# [2.3] Create a table ==> Refer to the Foundational portion of this project
...
{'TableDescription': {'TableName': 'MyMovieList', 'TableStatus': 'DELETING', 
 ...
 'ResponseMetadata': {'RequestId': 'E7EJOVKI7O1PTM5H10I4VIOGDBVV4KQNSO5AEMVJF66Q9ASUAAJG', 'HTTPStatusCode': 200, 
    'HTTPHeaders': {'server': 'Server', 'date': 'Wed, 19 Apr 2023 03:37:49 GMT', 
        'content-type': 'application/x-amz-json-1.0', 'content-length': '358', 'connection': 'keep-alive', 
        'x-amzn-requestid': 'E7EJOVKI7O1PTM5H10I4VIOGDBVV4KQNSO5AEMVJF66Q9ASUAAJG', 'x-amz-crc32': '787405542'}, 
'RetryAttempts': 0}}
'''

######### Memo ########
""" The following code block has not been tested yet. It will be tested later.
import boto3
from boto3.dynamodb.conditions import Key, Attr
dynamodb = boto3.resource('dynamodb') # Get the service resource.
table = dynamodb.Table('MyMovieList') # call MyMovieList DynamoDB table
response = table.query(
    ...
    KeyConditionExpression='genre = :g1',
